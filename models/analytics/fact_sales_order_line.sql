...
{% if is_incremental() %}
  WHERE 
    fact_line.last_edited_when >= (SELECT MAX(last_edited_when) FROM {{ this }})
    OR fact_header.sales__orders__last_edited_when >= (SELECT MAX(sales__orders__last_edited_when) FROM {{ this }})
{% endif %}
